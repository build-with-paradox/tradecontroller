    ...
    cell: (info) => {
      ...
      return (
        <div className="flex gap-2">
          <button
            className={`${
              isDisabled ? "bg-gray-300 cursor-not-allowed" : "bg-green-100 text-green-700 "
          ...
          <button
            className={`${
              isDisabled ? "bg-gray-300 cursor-not-allowed" : "bg-red-100 text-red-700 cursor-pointer"
...
const All_Products = () => {
  ...
  const handleApprove = (id: number) => {
    // Find the product and update its status to "approved"
    const updatedProducts = products.map((product) =>
      product.id === id ? { ...product, status: "approved" } : product
    );
    console.log("Approved Product:", updatedProducts);
  };
  ...
  return (
    <div className="bg-white shadow-md border border-gray-200 rounded-lg pl-3 pr-3  mt-3 ml-5 mx-4">
      <h1 className="text-2xl font-semibold text-gray-800 ml-3 mt-3">All Products</h1>
