...
const handleCheckIssue = (id: number) => {
...
const columns: ColumnDef<typeof products[0]>[] = [
    ...
    { id: "revenue", header: "Revenue", cell: (info) => `₹${info.row.original.revenue.toLocaleString()}` },
    {
        ...
        cell: (info) => {
            ...
            return (
                <button
                    className="bg-blue-100 text-blue-700 text-sm py-1 px-5 rounded-full whitespace-nowrap cursor-pointer"
                    onClick={() => handleCheckIssue(id)}
                >
                    Check Issue
                </button>
...
export default Reported_Products;
