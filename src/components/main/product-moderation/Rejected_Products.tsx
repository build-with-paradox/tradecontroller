...
import ReusableTable from "@/components/ui/ReusableTable";
  ...
  const handleReApprove = (id: number) => {
    console.log(`Product with id ${id} has been reapproved!`);
  };
...
const products = [
  ...
  },
  {
    ...
    status: "disapproved", // set status to disapproved for demonstration
  },
  // More products here...
];
...
const columns: ColumnDef<typeof products[0]>[] = [
  ...
  { id: "revenue", header: "Revenue", cell: (info) => `₹${info.row.original.revenue.toLocaleString()}` },
  {
    ...
    cell: (info) => {
      return (
        <button
          onClick={() => handleReApprove(info.row.original.id)}
          className="bg-blue-100 text-blue-700 text-md py-1 px-5 rounded-full whitespace-nowrap cursor-pointer"
        >
          Re-Approve
        </button>
      );
...
const Rejected_Products = () => {
  ...
  const filteredProducts = products.filter(
    (product) =>
      ...
      product.status === "disapproved" // Only show rejected products
  );


  return (
    <div className="bg-white shadow-md border border-gray-200 rounded-lg pl-3 pr-3 mt-5 ml-5 mx-4">
      <h1 className="text-2xl font-semibold text-gray-800 ml-3 mt-3">❌ Rejected/Removed Listings</h1>
      <p className="text-base text-gray-600 mt-2 ml-3">
        📊 Here's a snapshot of your <span className="text-gray-900 font-semibold">Rejected/Removed Listings</span> on your Platform.
      </p>
...
export default Rejected_Products;
